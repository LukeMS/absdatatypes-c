...
#include "sstruct_chtdict.h"


int sstruct_chtdict_hash(const void *data_, int buckets)
{
    const SSTRUCT_DictData *data = data_;
    return (int)(hashpjw(data->key, buckets));
}


int si_chtdict_match(const void *data1, const void *data2) {
    const SSTRUCT_DictData *dict_data_ptr_1 = data1;
    const SSTRUCT_DictData *dict_data_ptr_2 = data2;
    int res = !(bool)(strcmp(
...
void si_chtdict_new_node(SSTRUCT_DictData **node, char * key) {
...
int si_chtdict_get(CHTDict *dict, char * key) {
    SSTRUCT_DictData *match, *match_ref;
...
int si_chtdict_pop(CHTDict *dict)
{
    ...
    {
        return ((SSTRUCT_DictData *)data)->value;
    }
    return -1;
}
